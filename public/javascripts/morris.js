(function() {
  ...
  Morris.Line = (function() {
    ...
    Line.prototype.updateHover = function(index) {
      ...
      this.hoverSet.show();
      // HOOK FOR DATE FIX
      // ugly fix to show dates more euro friendly
      //
      // easy fix because else I would need to modify input structure, morris way of handling dates, etc.
      // this is just the labels

      // store and remove the year
      var labelDateFix = this.columnLabels[index].split('-');
      // remove trailing zero
      $.each(labelDateFix, function( i, dp ) {
        if( dp.substr( 0, 1 ) == 0 ) {
          labelDateFix[ i ] = dp.substr( 1 );
        }
      });
      // reverse to day-month
      labelDateFix = labelDateFix.reverse().join('-');
      // END DATE FIX

      this.xLabel.attr('text', labelDateFix);
      for (i = _i = 0, _ref = this.series.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
  ...
  Morris.LABEL_SPECS = {
    ...
    "day": {
      ...
      fmt: function(d) { 
        return "" + (Morris.pad2(d.getDate())) + "-" + (Morris.pad2(d.getMonth() + 1));
      },
      incr: function(d) {
